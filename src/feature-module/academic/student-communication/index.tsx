...
// Student chat — uses chat/inbox/ and chat/messages/
const StudentCommunication = () => {
  const studentId = localStorage.getItem("student_id");
  const roleId = localStorage.getItem("role_id");

  // Redirect teachers to the correct communication page
  useEffect(() => {
    if (roleId === "2") {
      window.location.href = "/academic/communication";
    }
  }, [roleId]);
  ...
  // ── Fetch inbox (teachers in student's class) ──
  ...
  const fetchMessages = useCallback(async (teacherId: number) => {
    if (!studentId) {
  ...
  // Poll every 4s when a teacher is selected
  useEffect(() => {
    clearInterval(pollRef.current);
    if (selected) {
  ...
  const handleSend = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!text.trim() || !selected || !studentId) return;
              ...
              {loadingInbox ? (
                ...
                filtered.map((t) => (
                  <div
                    ...
                    style={{
                      cursor: "pointer",
                      background: selected?.id === t.id ? "#f0f4ff" : "transparent",
                      borderLeft: selected?.id === t.id ? "3px solid #4a6fa5" : "3px solid transparent",
                      transition: "background 0.15s",
                    }}
                  >
                    <div className="flex-shrink-0">
                      {t.photo ? (
                        <img src={mediaUrl(t.photo)} className="rounded-circle"
                        ...
                        <div className="rounded-circle text-white d-flex align-items-center justify-content-center fw-bold"
                          style={{ width: 42, height: 42, fontSize: 15, background: "linear-gradient(135deg,#6b8cce,#4a6fa5)" }}>
                    ...
                    <div className="flex-grow-1 min-w-0">
                      ...
                      <small className="text-muted">
                        {Array.isArray(t.subjects) ? t.subjects.join(", ") : t.subjects || "Teacher"}
                      </small>
                    </div>
                  </div>
                ))
          ...
          <div className="col-xl-9 col-md-8 d-flex flex-column" style={{ height: "100%" }}>
            {selected ? (
                ...
                <div className="px-4 py-3 border-bottom d-flex align-items-center gap-3"
                  ...
                  {selected.photo ? (
                    ...
                    <div className="rounded-circle text-white d-flex align-items-center justify-content-center fw-bold flex-shrink-0"
                      style={{ width: 40, height: 40, fontSize: 14, background: "linear-gradient(135deg,#6b8cce,#4a6fa5)" }}>
                  ...
                  <div>
                    ...
                    <small className="text-muted">
                      {Array.isArray(selected.subjects) ? selected.subjects.join(", ") : selected.subjects || "Teacher"}
                    </small>
                  </div>
                </div>
              ...
              <div className="h-100 d-flex flex-column align-items-center justify-content-center text-muted">
                ...
                <p className="mt-2">Select a teacher to start chatting</p>
              </div>
            )}
          </div>
