...
export default function Register() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      // await axios.post("http://127.0.0.1:8000/school/register/", {
      await axios.post("https://api.schoolcaddie.com/school/register/", {
        ...form,
      ...
      Swal.fire({
        ...
        text: "Your school account has been created. Please login to continue.",
        icon: "success",
        ...
        if (result.isConfirmed) {
          navigate("/admin-login"); // Redirect to Login-2
        }
      });
    ...
    <div style={{ fontFamily: "'Poppins', sans-serif", backgroundColor: "#f8fafc", color: "#0f172a", overflowX: 'hidden' }}>
      ...
      <section className="py-5 position-relative" style={{ 
        ...
        <div className="container text-center">
          ...
          <p className="lead text-white opacity-90 mx-auto" style={{ maxWidth: "700px" }}>
            Just a few more details to set up your school dashboard and start managing your institution effectively.
          </p>
        </div>
      </section>
              ...
              <div className="card-body p-4 p-md-5 bg-white">
                ...
                {sessionId && (
                  <div className="alert alert-success d-flex align-items-center rounded-3 mb-4" role="alert">
                    <i className="ti ti-circle-check-filled fs-4 me-2"></i>
                    <div><strong>Payment Verified!</strong> Please complete your account details.</div>
                  </div>
                )}

                <form onSubmit={submit} className="registration-form">
                  <div className="mb-4 text-center">
                    <h4 className="fw-bold">Create Admin Account</h4>
                    <p className="text-muted small">Fill in the information below</p>
                  </div>

                  {/* School Name */}
                  <div className="mb-4">
                    <div className="form-floating shadow-sm rounded-3 overflow-hidden position-relative">
                      <input
                        type="text"
                      ...
                      />
                      <label htmlFor="schoolName" className="text-muted" style={{ paddingLeft: '45px' }}>School Name</label>
                      <i className="ti ti-school position-absolute top-50 start-0 translate-middle-y ms-3 text-primary fs-5"></i>
                    </div>
                  </div>
                  ...
                  <div className="mb-4">
                    <div className="form-floating shadow-sm rounded-3 overflow-hidden position-relative">
                      ...
                      />
                      <label htmlFor="adminEmail" className="text-muted" style={{ paddingLeft: '45px' }}>Admin Email Address</label>
                      <i className="ti ti-mail position-absolute top-50 start-0 translate-middle-y ms-3 text-primary fs-5"></i>
                    </div>
                  </div>
                  ...
                  <div className="mb-4">
                    <div className="form-floating shadow-sm rounded-3 overflow-hidden position-relative">
                      ...
                      />
                      <label htmlFor="password" className="text-muted" style={{ paddingLeft: '45px' }}>Create Password</label>
                      <i className="ti ti-lock position-absolute top-50 start-0 translate-middle-y ms-3 text-primary fs-5"></i>
                    </div>
                  </div>
                  ...
                  <button 
                    type="submit" 
                    disabled={loading}
                    className="btn btn-primary btn-lg w-100 rounded-pill fw-bold shadow-sm py-3 mt-3 border-0 transition-all"
                  >
                    {loading ? (
                ...
                <div className="text-center mt-4">
                  <p className="small text-muted mb-0">
                    ...
                    <Link to="/admin-login" className="text-primary text-decoration-none fw-bold ms-1">Login here</Link>
                  </p>
                </div>
              </div>
            </div>
  ...
  <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', backgroundColor: 'rgba(15, 23, 42, 0.8)', backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999, padding: '20px' }} onClick={() => setShowSupport(false)}>
    <div className="fade-in" style={{ background: 'white', borderRadius: '40px', width: '100%', maxWidth: '800px', padding: '50px', position: 'relative', boxShadow: '0 50px 100px rgba(0,0,0,0.4)', border: '1px solid rgba(255,255,255,0.2)' }} onClick={(e) => e.stopPropagation()}>
      ...
      <style>{`
        ...
        .transition-all:hover:not(:disabled) { transform: translateY(-2px); box-shadow: 0 5px 15px rgba(13, 110, 253, 0.3) !important; }
      `}</style>
    </div>
  );
}